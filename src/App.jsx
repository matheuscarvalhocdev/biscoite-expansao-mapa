...
import { LogIn, Plus, X, MapPin, Save, LogOut, Search, Navigation, Bell } from 'lucide-react';
...
// --- COMPONENTE MÁGICO: Controlador da Câmera do Mapa ---
function MapController({ coords }) {
  const map = useMap();
  useEffect(() => {
    if (coords) {
      // O flyTo cria aquela animação de "voo" cinematográfica
...
export default function App() {
  ...
  // Novos Estados para a Animação e Notificação
  ...
  const handleLogin = (e) => {
    ...
    setIsLoggedIn(true);
    setShowLogin(false);
    showNotification("Acesso Liberado", "Bem-vindo ao painel de expansão Biscoitê.");
  };
  ...
  const handleAddPonto = (e) => {
    ...
    showNotification("Sucesso", `${pontoFinal.nome} adicionado ao mapa.`);
    setMapCenter([pontoFinal.lat, pontoFinal.lng]);
  };

  // Função para voar até o ponto e chamar a notificação
  const handleLocationClick = (local) => {
    setMapCenter([local.lat, local.lng]);
    showNotification("Localização Rastreada", `Voando para ${local.nome} em ${local.cidade}`);
  };

  // Função que controla a Notificação (some após 4 segundos)
  ...
  if (showLogin && !isLoggedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-b-dark relative overflow-hidden">
        <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-b-primary rounded-full blur-[150px] opacity-20"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-b-secondary rounded-full blur-[150px] opacity-20"></div>
        
        <div className="bg-b-dark/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-b-primary/30 z-10">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="email" placeholder="E-mail corporativo" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" required />
            <input type="password" placeholder="Senha" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" required />
            <button type="submit" className="w-full bg-b-primary text-b-dark p-4 rounded-xl font-bold hover:bg-b-secondary transition-all shadow-lg shadow-b-primary/20">ENTRAR</button>
            <button type="button" onClick={() => setShowLogin(false)} className="w-full text-b-secondary text-sm font-medium hover:text-b-light transition-colors">Voltar ao mapa</button>
          </form>
        </div>
      </div>
    );
  ...
  // --- INTERFACE PRINCIPAL ---
  return (
    <div className="min-h-screen bg-b-dark p-4 md:p-8 font-sans flex flex-col relative overflow-hidden">
      
      {/* TOAST DE NOTIFICAÇÃO (Estilo Profissional/Vidro Fosco) */}
      {toast.show && (
        <div className="fixed bottom-8 right-8 z-[9999] bg-b-dark/90 backdrop-blur-xl border border-b-primary/40 text-white p-4 pr-6 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] flex items-center gap-4 animate-in slide-in-from-right-8 fade-in duration-300">
          ...
          <button onClick={() => setToast({show: false})} className="ml-2 text-b-secondary/50 hover:text-white transition-colors">
            <X size={16} />
          </button>
        </div>
      )}
      ...
      {/* HEADER */}
      <header className="mb-6 flex justify-between items-end border-b border-b-primary/20 pb-4">
        <div>
          <LogoBiscoite />
          <p className="text-b-secondary mt-2 font-medium text-sm md:text-base tracking-wide">Mapeamento Estratégico de Novas Franquias</p>
        </div>
        
        {isLoggedIn ? (
          <div className="flex gap-3">
            <button onClick={() => setShowModal(true)} className="bg-b-primary text-b-dark px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-b-primary/20 hover:bg-b-secondary transition-all">
              <Plus size={18} /> NOVO PONTO
            </button>
            <button onClick={() => { setIsLoggedIn(false); showNotification("Sessão Encerrada", "Você saiu do sistema."); }} className="bg-white/5 text-b-secondary p-2.5 rounded-xl border border-b-primary/30 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/50 transition-all">
              <LogOut size={20} />
            </button>
          </div>
        ) : (
          <button onClick={() => setShowLogin(true)} className="text-b-secondary border border-b-primary/30 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-b-primary/10 hover:text-b-light transition-all">
            <LogIn size={20} /> ACESSO EQUIPE
          </button>
        )}
      </header>

      {/* CONTEÚDO: BARRA LATERAL E MAPA */}
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 max-w-[1400px] mx-auto w-full relative z-10">
        
        <aside className="bg-b-dark/50 backdrop-blur-md rounded-3xl shadow-2xl border border-b-primary/30 p-5 flex flex-col h-[78vh] overflow-hidden">
          <div className="relative mb-5">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-b-secondary/60" size={18} />
            <input 
              type="text" placeholder="Buscar cidade ou shopping..." 
              className="w-full pl-10 p-3 bg-white/5 border border-b-primary/30 rounded-xl text-sm text-b-light focus:outline-none focus:border-b-secondary transition-all"
          ...
          <div className="overflow-y-auto space-y-3 flex-1 pr-2 custom-scrollbar">
            {locais.filter(l => l.nome.toLowerCase().includes(busca.toLowerCase()) || l.cidade.toLowerCase().includes(busca.toLowerCase())).map(local => (
              <div 
                key={local.id} 
                onClick={() => handleLocationClick(local)} // <- O Clique Mágico aqui!
                className="p-4 border border-b-primary/20 rounded-2xl hover:border-b-secondary/50 transition-all cursor-pointer bg-white/5 hover:bg-white/10 group relative overflow-hidden"
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-bold px-2 py-1 bg-b-dark text-b-secondary rounded-md border border-b-primary/30">
                    {local.regiao}
                  </span>
                  <span className={`text-[9px] font-bold px-2 py-1 rounded-md uppercase tracking-wider border ${
                    local.status === 'Alta Prioridade' ? 'bg-b-primary/20 text-b-light border-b-primary/50' : 
                    local.status === 'Em Negociação' ? 'bg-orange-500/20 text-orange-300 border-orange-500/50' : 
                    'bg-green-500/20 text-green-300 border-green-500/50'
                  }`}>
                    {local.status}
                  </span>
                </div>
                <h4 className="font-bold text-b-light text-base group-hover:text-b-secondary transition-colors">{local.nome}</h4>
                <p className="text-xs text-b-secondary/70 mt-1 flex items-center gap-1"><MapPin size={12}/> {local.cidade}</p>
                
                {/* Ícone sutil no hover */}
                <Navigation size={16} className="absolute right-4 bottom-4 text-b-primary opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            ))}
          </div>
        </aside>

        <section className="lg:col-span-3 bg-b-dark/30 rounded-3xl shadow-2xl border border-b-primary/30 p-2 h-[78vh] relative z-0 overflow-hidden">
          <MapContainer center={[-15.79, -47.88]} zoom={5} className="w-full h-full rounded-2xl">
            <TileLayer url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" />
            
            {/* O Controlador injetado dentro do mapa para fazer a câmera voar */}
              ...
              <Marker key={local.id} position={[local.lat, local.lng]}>
                <Popup className="custom-popup">
                  <div className="text-center font-sans p-1">
                    <p className="font-bold text-b-dark text-sm mb-1">{local.nome}</p>
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider bg-b-primary text-white">
                      {local.status}
                    </span>
                  </div>
                </Popup>
      ...
      {/* MODAL DARK THEME */}
      {showModal && (
        <div className="fixed inset-0 bg-b-dark/80 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
          <div className="bg-b-dark w-full max-w-2xl rounded-3xl shadow-2xl border border-b-primary/30 overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="bg-white/5 border-b border-b-primary/30 p-6 text-b-light flex justify-between items-center">
              <h3 className="text-xl font-bold flex items-center gap-2 text-b-secondary"><MapPin /> Cadastrar Nova Praça</h3>
              <button onClick={() => setShowModal(false)} className="text-b-secondary hover:text-white hover:rotate-90 transition-all"><X /></button>
            </div>
            
            <form onSubmit={handleAddPonto} className="p-6 grid grid-cols-2 gap-5">
              <div className="col-span-2">
                <label className="text-xs font-bold text-b-secondary uppercase tracking-wider mb-2 block">Nome do Shopping / Local</label>
                <input required value={novoPonto.nome} onChange={e => setNovoPonto({...novoPonto, nome: e.target.value})} type="text" className="w-full p-3 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" placeholder="Ex: Shopping Iguatemi" />
              </div>
              <div>
                <label className="text-xs font-bold text-b-secondary uppercase tracking-wider mb-2 block">Cidade e UF</label>
                <input required value={novoPonto.cidade} onChange={e => setNovoPonto({...novoPonto, cidade: e.target.value})} type="text" className="w-full p-3 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" placeholder="Ex: São Paulo - SP"/>
              </div>
              <div>
                <label className="text-xs font-bold text-b-secondary uppercase tracking-wider mb-2 block">Status Estratégico</label>
                <select value={novoPonto.status} onChange={e => setNovoPonto({...novoPonto, status: e.target.value})} className="w-full p-3 bg-b-dark border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all">
              ...
              <div>
                <label className="text-xs font-bold text-b-secondary uppercase tracking-wider mb-2 block">Latitude</label>
                <input required value={novoPonto.lat} onChange={e => setNovoPonto({...novoPonto, lat: e.target.value})} type="text" className="w-full p-3 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" placeholder="-23.5505" />
              </div>
              <div>
                <label className="text-xs font-bold text-b-secondary uppercase tracking-wider mb-2 block">Longitude</label>
                <input required value={novoPonto.lng} onChange={e => setNovoPonto({...novoPonto, lng: e.target.value})} type="text" className="w-full p-3 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all" placeholder="-46.6333" />
              </div>
              <div className="col-span-2 pt-4">
                <button type="submit" className="w-full bg-b-primary text-b-dark p-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-b-secondary transition-all shadow-lg shadow-b-primary/20">
                  <Save size={20} /> ADICIONAR AO MAPA
                </button>
              </div>
            </form>
          </div>
